package student

import (
	"Template/internal/entity"
	"Template/pkg/log"
	"context"
	"fmt"

	"github.com/ttacon/libphonenumber"

	"regexp"
	"strconv"
	"strings"
	"time"
)

type Service interface {
	Create(ctx context.Context, input []CreateStudentRequest) entity.Result
	//Query(ctx context.Context, page, limit int, classId, semester, year *int) entity.Result
	// Get(ctx context.Context, id int) entity.Result
	//Update(ctx context.Context, id string, input UpdateClassRequest) entity.Result
	//Delete(ctx context.Context, id string) entity.Result
}

type CreateStudentRequest struct {
	Id               string                  `json:"id"`
	Name             string                  `json:"name"`
	DateOfBirth      time.Time               `json:"dateOfBirth"`
	Gender           string                  `json:"gender"`
	Course           int                     `json:"course"`
	FacultyID        string                  `json:"faculty"`
	ProgramID        string                  `json:"program"`
	StatusID         string                  `json:"status"`
	Phone            string                  `json:"phone"`
	Email            string                  `json:"email"`
	TemporaryAddress string                  `json:"temporaryAddress"`
	MailingAddress   string                  `json:"mailingAddress"`
	PermanentAddress string                  `json:"permanentAddress"`
	Nationality      string                  `json:"nationality"`
	Identity         entity.IdentityDocument `json:"identity"`
}

type UpdateStudentRequest struct {
	Id          string    `json:"id"`
	Name        string    `json:"name"`
	DateOfBirth time.Time `json:"dateOfBirth"`
}

type UpdateClassRequest struct {
	Id          string    `json:"classId"`
	Year        int       `json:"academicYear"`
	CourseId    string    `json:"courseId"`
	Semester    int       `json:"semester"`
	TeacherName string    `json:"teacherName"`
	MaxStudents int       `json:"maxStudents"`
	Room        string    `json:"room"`
	DayOfWeek   int       `json:"dayOfWeek"`
	StartTime   float64   `json:"startTime"`
	EndTime     float64   `json:"endTime"`
	Deadline    time.Time `json:"deadline"`
}

type GetClassRequest struct {
	Id          string `json:"classId"`
	Year        int    `json:"academicYear"`
	CourseId    string `json:"courseId"`
	CourseName  string `json:"courseName"`
	Semester    int    `json:"semester"`
	TeacherName string `json:"teacherName"`
	MaxStudents int    `json:"maxStudents"`
	//current students
	Room      string    `json:"room"`
	DayOfWeek int       `json:"dayOfWeek"`
	StartTime float64   `json:"startTime"`
	EndTime   float64   `json:"endTime"`
	Deadline  time.Time `json:"deadline"`
}

type GetAllClassRequest struct {
	Data  []GetClassRequest `json:"data"`
	Total int               `json:"total"`
}

type service struct {
	repo   Repository
	logger log.Logger
}

func NewService(repo Repository, logger log.Logger) Service {
	return service{repo, logger}
}

func (s service) Create(ctx context.Context, students []CreateStudentRequest) entity.Result {
	allValid := true
	index := -1
	errors := make([]entity.APIError, 0)
	for _, student := range students {
		index++
		if validRes, code, msg := validate(s, student, false); !validRes {
			allValid = false
			errors = append(errors, entity.APIError{
				Code:    code,
				Message: msg,
				Index:   &index,
			})
			continue
		}

		if validRes, code, msg := validateStudent(s, student, false); !validRes {
			allValid = false
			errors = append(errors, entity.APIError{
				Code:    code,
				Message: msg,
				Index:   &index,
			})
			continue
		}
	}
	if !allValid {
		return entity.Fail("ADD_STUDENTS_FAILED", "Thêm sinh viên thất bại", errors)
	}
	for _, student := range students {
		facultyId, _ := strconv.Atoi(student.FacultyID)
		programId, _ := strconv.Atoi(student.ProgramID)
		statusId, _ := strconv.Atoi(student.StatusID)
		studentEntity := entity.Student{
			Name:             student.Name,
			DateOfBirth:      student.DateOfBirth,
			Gender:           student.Gender,
			Email:            student.Email,
			Phone:            student.Phone,
			PermanentAddress: student.PermanentAddress,
			TemporaryAddress: student.TemporaryAddress,
			MailingAddress:   student.MailingAddress,
			Course:           student.Course,
			ProgramID:        programId,
			StatusID:         statusId,
			FacultyID:        facultyId,
			Nationality:      student.Nationality,
			IsDeleted:        false,
		}
		if id, err := s.repo.Create(ctx, studentEntity); err != nil {
			return entity.Fail("ADD_STUDENT_FAILED", err.Error(), nil)
		} else {
			if err := s.repo.CreateIdentityDocument(ctx, entity.IdentityDocument{
				StudentID:  id,
				Type:       student.Identity.Type,
				Number:     student.Identity.Number,
				IssuedDate: student.Identity.IssuedDate,
				ExpiryDate: student.Identity.ExpiryDate,
				IssuePlace: student.Identity.IssuePlace,
				Country:    student.Identity.Country,
				IsChip:     student.Identity.IsChip,
				Note:       student.Identity.Note,
			}); err != nil {
				return entity.Fail("ADD_IDENTITY_DOCUMENT_FAILED", err.Error(), nil)
			}
		}
	}
	return entity.Ok(students, nil)
}

func validate(s service, student CreateStudentRequest, isUpdate bool) (isValid bool, code string, message string) {
	if (!isUpdate && student.Course != 0) && !(student.Course != 0 && student.Course >= 2000) {
		return false, "INVALID_COURSE", "Khóa học không hợp lệ."
	}
	emailSetting, _ := s.repo.GetEmailSetting(context.Background())
	fmt.Println("Email Setting:", emailSetting)
	isValidEmail := func(email string) bool {
		emailDomain := emailSetting.EmailDomain
		emailPattern := emailSetting.EmailPattern
		re := regexp.MustCompile(emailPattern)
		if !re.MatchString(email) {
			return false
		}
		domain := email[strings.LastIndex(email, "@"):]
		fmt.Println("Domain:", domain)
		fmt.Println("Email Domain:", emailDomain)
		return domain == emailDomain
	}
	fmt.Println("Email:", student.Email)
	if (!isUpdate || student.Email != "") && (student.Email != "" && !isValidEmail(student.Email)) {
		return false, "INVALID_EMAIL", "Email không đúng định dạng."
	}
	phone, err := libphonenumber.Parse(student.Phone, "VN")
	if err != nil {
		return false, "INVALID_PHONE", "Số điện thoại không hợp lệ."
	}
	if (!isUpdate || student.Phone != "") && (student.Phone != "" && !libphonenumber.IsValidNumber(phone)) {
		return false, "INVALID_PHONE", "Số điện thoại không hợp lệ."
	}
	return true, "", ""
}

func validateStudent(s service, student CreateStudentRequest, isUpdate bool) (isValid bool, code string, message string) {
	if (!isUpdate || student.Email != "") && !s.repo.CheckNoExistingEmail(context.Background(), student.Email) {
		return false, "DUPLICATE_EMAIL", "Email đã tồn tại."
	}
	if (!isUpdate || student.Phone != "") && !s.repo.CheckNoExistingPhone(context.Background(), student.Phone) {
		return false, "DUPLICATE_PHONE", "Số điện thoại đã tồn tại."
	}
	if (!isUpdate || student.FacultyID != "") && !s.repo.CheckValidFaculty(context.Background(), student.FacultyID) {
		return false, "INVALID_FACULTY", "Mã khoa không hợp lệ."
	}
	if (!isUpdate || student.ProgramID != "") && !s.repo.CheckValidProgram(context.Background(), student.ProgramID) {
		return false, "INVALID_PROGRAM", "Mã chương trình không hợp lệ."
	}
	if (!isUpdate || student.StatusID != "") && !s.repo.CheckValidStatus(context.Background(), student.StatusID) {
		return false, "INVALID_STATUS", "Mã trạng thái không hợp lệ."
	}
	return true, "", ""
}

// func (s service) Query(ctx context.Context, page, limit int, classId, semester, year *int) entity.Result {
// 	classes, err := s.repo.Query(ctx, page, limit, classId, semester, year)
// 	if err != nil {
// 		return entity.Fail("GET_CLASSES_FAILED", err.Error(), nil)
// 	}
// 	res := make([]GetClassRequest, len(classes))
// 	for i := range classes {
// 		res[i] = GetClassRequest{
// 			Id:          strconv.Itoa(classes[i].ID),
// 			Year:        classes[i].AcademicYear,
// 			CourseId:    strconv.Itoa(classes[i].CourseID),
// 			CourseName:  classes[i].Course.Name,
// 			Semester:    classes[i].Semester,
// 			TeacherName: classes[i].TeacherName,
// 			MaxStudents: classes[i].MaxStudents,
// 			Room:        classes[i].Room,
// 			DayOfWeek:   classes[i].DayOfWeek,
// 			StartTime:   classes[i].StartTime,
// 			EndTime:     classes[i].EndTime,
// 			Deadline:    classes[i].DeadLine,
// 		}
// 	}
// 	total := len(res)
// 	return entity.Ok(GetAllClassRequest{
// 		Data:  res,
// 		Total: total,
// 	}, nil)
// }

// func (s service) Update(ctx context.Context, id string, classReq UpdateClassRequest) entity.Result {
// 	intID, _ := strconv.Atoi(id)
// 	class, err := s.repo.Get(ctx, intID)
// 	if errors.Is(err, gorm.ErrRecordNotFound) {
// 		return entity.Fail("CLASS_NOT_FOUND", "Lớp học không tồn tại.", nil)
// 	}
// 	courseId, _ := strconv.Atoi(classReq.CourseId)

// 	err = s.repo.Update(ctx, entity.Class{
// 		ID:           intID,
// 		AcademicYear: classReq.Year,
// 		CourseID:     courseId,
// 		Semester:     classReq.Semester,
// 		TeacherName:  classReq.TeacherName,
// 		MaxStudents:  classReq.MaxStudents,
// 		Room:         classReq.Room,
// 		DayOfWeek:    classReq.DayOfWeek,
// 		StartTime:    classReq.StartTime,
// 		EndTime:      classReq.EndTime,
// 		DeadLine:     classReq.Deadline,
// 		IsDeleted:    class.IsDeleted,
// 	})
// 	if err != nil {
// 		return entity.Fail("UPDATE_CLASS_FAILED", err.Error(), nil)
// 	}
// 	return entity.Ok(classReq, nil)
// }

// func (s service) Delete(ctx context.Context, id string) entity.Result {
// 	err := s.repo.Delete(ctx, id)
// 	if err != nil {
// 		if errors.Is(err, gorm.ErrRecordNotFound) {
// 			return entity.Fail("COURSE_NOT_FOUND", "Lớp học không tồn tại.", nil)
// 		}
// 		return entity.Fail("DELETE_COURSE_FAILED", err.Error(), nil)
// 	}
// 	getClassRequest := GetClassRequest{
// 		Id: id,
// 	}
// 	return entity.Ok(getClassRequest, nil)
// }
