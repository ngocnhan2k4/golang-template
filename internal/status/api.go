...
func (r resource) create(c *gin.Context) {
	var status CreateStatusRequest
	err := c.ShouldBind(&status)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	res := r.service.Create(c.Request.Context(), status)
	if res.ErrorMessage != "" {
		err := entity.APIError{
			Code:    res.ErrorCode,
			Message: res.ErrorMessage,
...
func (r resource) update(c *gin.Context) {
	id := c.Param("id")
	var status UpdateStatusRequest
	err := c.ShouldBind(&status)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	res := r.service.Update(c.Request.Context(), id, status)
	if res.ErrorMessage != "" {
		err := entity.APIError{
			Code:    res.ErrorCode,
			Message: res.ErrorMessage,
